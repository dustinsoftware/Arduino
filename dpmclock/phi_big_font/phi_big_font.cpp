...
void render_big_char_top(char ch)
{
  ...
  {
    for (byte i=0; i<3; i++)
    if (!inverted_big_font) output_lcd->write(font_35_sym[i]);
    else output_lcd->write(7-font_35_sym[i]);
  }
...
void render_big_char_bot(char ch)
{
  ...
  {
    for (byte i=0; i<3; i++)
    if (!inverted_big_font) output_lcd->write(font_35_sym[i+3]);
    else output_lcd->write(7-font_35_sym[i+3]);
  }
  if (!inverted_big_font) output_lcd->write(' ');
  else output_lcd->write(255);
}
